...
Remove duplicates in-place from a sorted array so each unique element appears once.
...
After removing duplicates, return the number of unique elements `k`. The first `k` elements of `nums` should contain the unique numbers in sorted order. The remaining elements beyond index `k - 1` can be ignored.
...
Your implementation is tested with the following logic:
